...
pub fn seq_matrix_mult(m1: Vec<Vec<f64>>, m2: Vec<Vec<f64>>) -> Vec<Vec<f64>> {
...
mod tests {
    ...
    fn dot_test() {
        ...
        let big_vector = (0..=20_000_000).map(|a| a as f64).collect::<Vec<_>>();
        ...
        let (output, time) = timed(|| seq_dot_product(&big_vector, &big_vector));
        println!("sequential dot product with 20_000_000-size   output: {:?}    time: {:?}", output, time);

        let (output, time) = timed(|| par_dot_product(&big_vector, &big_vector));
